...
# scripts expects two cli arguments: 
#   $1 is IP:port
#   $2 delay in seconds between probes
# change defaults here
# OpenSSL requires the port number formatte as IP:port, taken from arg $1; default localhost:443
...
if [[ "$result" =~ "SSLv3, Cipher is" ]] ; then 
  ...
  echo NO, SSLv3 is not supported, Test OK 
fi 
